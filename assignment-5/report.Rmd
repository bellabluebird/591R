...
var_data <- filter_zero_var_genes(data)
...
no_genes <- var_data %>% select(-gene)  #removing the gene column
#processing each of the column names from this row
meta <- meta_info_from_labels(colnames(no_genes))
```
...
library_size_data <- get_library_size(var_data)
#Make a barplot showing the number of millions of reads for each sample
  p <- ggplot(library_size_data, aes(x = sample, y = value)) +
    geom_bar(stat = "identity", fill = "lightgreen") +
    labs(title = "Library Size (in Millions of Reads)",
         x = "Sample",
...
cpm_data <- normalize_by_cpm(var_data)
print(head(cpm_data, 10))
```
...
deseq_data <- deseq_normalize(var_data, meta)
print(head(deseq_data, 10))
```
...
boxplot <- plot_sample_distributions(var_data, scale_y_axis=TRUE)
print(boxplot)
```
...
plot_sample_distributions(cpm_data, scale_y_axis = TRUE)
plot_variance_vs_mean(cpm_data, scale_y_axis = TRUE)
plot_pca(cpm_data, meta)
```
...
plot_sample_distributions(deseq_data, scale_y_axis = TRUE)
plot_variance_vs_mean(deseq_data, scale_y_axis = TRUE)
plot_pca(deseq_data, meta)
```
