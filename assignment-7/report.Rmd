...
deseq_res <- run_deseq(counts_df, coldata, 10, "condition_day0_vs_adult")
cat("deseq results:")
head(deseq_res)
```
...
cat("group: ")
print(group)
limma_res <- run_limma(counts_df, design, group)
cat("Limma results:")
head(limma_res)
```
...
print(str(limma_res))
cat("head(deseq_res), notice the order of the p-values nad p-adjusted values:")
head(deseq_res)
```
...
new_combine_pval(deseq_res, edger_res, limma_res) %>%
  ggplot(aes(pval)) +
...
vooml <- voomless(counts_df, design, group)
